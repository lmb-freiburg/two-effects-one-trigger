...
import sys
import os
from .custom_transforms import sparsify_caption_drop_continous_half, sparsify_caption_drop_continous_quater
...
def create_model_and_transforms(
        ...
        cache_dir: Optional[str] = None,
        output_dict: Optional[bool] = None,
        text_transform_train: Optional[str] | None = None,
        text_transform_val: Optional[str] | None = None,
        drop_text_prob: Optional[float] = 0.0,           
        **model_kwargs,
):
    ...
    text_transform_train_fn = None
    text_transform_val_fn = None    
    if not text_transform_train is None:
        if text_transform_train == 'drop_half_caption':
            text_transform_train_fn = torchvision.transforms.Compose(
                [
                sparsify_caption_drop_continous_half(p=drop_text_prob)
                ]
            )            
        if text_transform_train == 'drop_3quater_caption':            
            text_transform_train_fn =  torchvision.transforms.Compose(
                [
                sparsify_caption_drop_continous_quater(p=drop_text_prob)
                ]
            )    
    if not text_transform_val is None:
        if text_transform_val == 'drop_half_caption':
            text_transform_val_fn = torchvision.transforms.Compose(
                [
                sparsify_caption_drop_continous_half(p=drop_text_prob)
                ]
            )            
        if text_transform_val == 'drop_3quater_caption':
            text_transform_val_fn = torchvision.transforms.Compose(
                [
                sparsify_caption_drop_continous_quater(p=drop_text_prob)
                ]
    ...
    return model, preprocess_train, preprocess_val, text_transform_train_fn, text_transform_val_fn
