...
from visiontext.distutils import get_world_size, print_with_rank
...
def get_wds_dataset(args, preprocess_img, text_transform, is_train, epoch=0, floor=False, tokenizer=None):
    ...
    if is_train:
        ...
        num_samples = args.val_num_samples or CC12M_SPLIT_SIZES['val']
    ...
    if not text_transform is None:
        pipeline.extend([
                    wds.map_dict(text=text_transform),
        ])
...
def get_indexable_wds_dataset(args, preprocess_img, text_transform, is_train, epoch=0, floor=False, tokenizer=None):
    if is_train:
        raise NotImplementedError
    dataset = IndexableCC12M(split='val', transform=preprocess_img, max_datapoints=None)    
    dataloader = DataLoader(
            dataset,
            batch_size=args.batch_size,
            num_workers=args.workers,
...
class IndexableCC12M(Dataset):
    def __init__(self, split="train", transform=None, max_datapoints=None):
        worker_id = get_worker_id()
        base_dir, tar_files = get_cc12m_clean(split)
        lookup = TarLookup(
            base_dir,
            tar_files,
    ...
    def __getitem__(self, item):
        ...
        try:
            image_arr = decode_jpeg(image_bytes)
...
def get_dataset_fn(data_path, dataset_type):
    ...
    elif dataset_type == "webdata_iter":
        return get_indexable_wds_dataset
...
def get_data(args, preprocess_fns, text_transform, epoch=0, tokenizer=None):
    preprocess_train, preprocess_val = preprocess_fns
    text_transform_train, text_transform_val = text_transform
    data = {}
    if not text_transform_val is None:
        raise NotImplementedError('text transform val not supported')

    if args.train_data or args.dataset_type == "synthetic":
        data["train"] = get_dataset_fn(args.train_data, args.dataset_type)(
            args, preprocess_train, text_transform=text_transform_train, is_train=True, epoch=epoch, tokenizer=tokenizer)

    if args.val_data:
        data["val"] = get_dataset_fn(args.val_data, 'webdata_iter')(
            args, preprocess_val, text_transform=text_transform_val, is_train=False, tokenizer=tokenizer)
