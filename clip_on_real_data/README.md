# Two Effects, One Trigger: On the Modality Gap, Object Bias, and Information Imbalance in Contrastive Vision-Language Models --- Real Data Experiments

We build on top of the OpenClip repository for these experiments. Please refer to the [original readme](#openclip) below for information on how to set up the environment etc. Here we only describe the experimental setups and the arguments introduced by us.

## Python environment setup
You can either start by following the OpenClip instructions and add the few extra packages required by our codebase, or get an exact copy of our environment by installing the packages listed in the environment.yml.

# Information Imbalance Experiments (CC12M)

We provide the arguments used to in our paper below. Please set the path to CC12M folders manually. We asume ./src to be the current working directory

## Train with full caption

```
torchrun --nproc_per_node 8 -m open_clip_train.main \
    --save-frequency 1 \
    --report-to tensorboard \
    --train-data "./data/cc12m/train/shard_{0000..00511}.tar" \
    --train-num-samples 9263104 \
    --dataset-type webdataset \
    --batch-size 110 \
    --precision amp \
    --workers 8 \
    --val-data "./data/cc12m/tar_val/shard_{00000..00031}.tar" \
    --save-frequency 1 \
    --report-to tensorboard \
    --warmup 10000 \
    --epochs 30 \
...
## Train with quarter of a caption
```
torchrun --nproc_per_node 8 -m open_clip_train.main \
    --save-frequency 1 \
    --report-to tensorboard \
    ...
    --local-loss \
    --gather-with-grad \
    --text_transform_train drop_3quater_caption \
    --drop_text_prob 1.0 \
    --resume latest
```
...
## Finetuing on higher imbalance data
Copy the checkpoint obtained using "full caption" training and copy it manually in a new folder for model initialization.
...
torchrun --nproc_per_node 8 -m open_clip_train.main \
    ...
    --epochs 10 \
    --model RN50 \
    --name RN50_full_finetune_learnable_temp \
    --lr 1e-3 \
    --accum-freq 1 \
    --local-loss \
    --gather-with-grad \
...
## Frozen Temperature
Copy the checkpoint obtained using "full caption" training and copy it manually in the new folder for model initialization.
...
torchrun --nproc_per_node 8 -m open_clip_train.main \
    ...
    --epochs 10 \
    --model RN50 \
    --name RN50_full_finetune_frozen_temp \
    --lr 1e-3 \
    --accum-freq 1 \
    --local-loss \
    --gather-with-grad \
    --text_transform_train drop_3quater_caption \
    --drop_text_prob 1.0 \
    --resume init_with_pretrained \
    --freeze_temp
```

# Finetuning Experiments on DCI

1. Download and prepare DCI following the instructions from the [official repo of DCI](https://github.com/facebookresearch/DCI)

2. Prepare captions, via `dci_finetuning_caption_generation.py`

3. Run fine-tuning, via
* for OpenAI's CLIP ViT-B/16:

    ```bash
    torchrun --nproc_per_node 4 -m open_clip_train.main \
        --dataset-type csv \
        --train-data "dci_short.csv" \
        --save-frequency 1 \
        --report-to tensorboard \
        --precision amp \
        --workers 8 \
        ...
        --model ViT-B-16 \
        --pretrained openai \
        --name finetune_openai_ViT-B-16_dcis_1e-5 \
        --accum-freq 1 \
        --local-loss \
        --gather-with-grad \
        --logs "logs" \
...
* for SigLIP ViT-B/16:
    ...
    torchrun --nproc_per_node 4 -m open_clip_train.main \
        ...
        --pretrained webli \
        --name finetune_webli_ViT-B-16_dcis_1e-5 \
        --accum-freq 1 \
        --local-loss \
        --gather-with-grad \
        --logs "logs" \
...
4. Evaluate, via `python dci_evaluation --models $MODEL_DIR`
